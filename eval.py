...
def main():
    corpus_train = read_conll("./corpus/corpus_eval/fr_sequoia-ud-train.conllu")
    vocab_train = build_vocabulaire(corpus_train)
    for model_name in ("spacy_model2/model-best", "fr_core_news_sm", "fr_core_news_md", "fr_core_news_lg"):
        print(model_name)
        model_spacy = spacy.load(model_name)
        corpus_gold = read_conll("./corpus/corpus_eval/fr_sequoia-ud-test.conllu", vocabulaire=vocab_train)
