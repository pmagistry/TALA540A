...
def sequoia_conllu(fichier) -> SentenceList:
    ...
    """
    if fichier == "train" :
        with open(f"./corpus/fr_sequoia-ud-train.conllu", "r", encoding="utf-8") as file:
            data = file.read()
    else :
...
def gsd_conllu(fichier) -> SentenceList:
    ...
    """
    if fichier == "train" :
        with open(f"./corpus/zh_gsdsimp-ud-train.conllu", "r", encoding="utf-8") as file:
            data = file.read()
    else :
        with open(f"./corpus/zh_gsdsimp-ud-test.conllu", "r", encoding="utf-8") as file:
            data = file.read()
    return parse(data)


def get_conllu(langue: str, fichier: str, vocabulaire: Optional[Set[str]] = None) -> Corpus:
    """
    Args:
