        ...
        for line in f:
            ...
            if not line.startswith("#"):
                if line == "":
                    ...
                    fields = line.split("\t")
                    form, tag = fields[0], fields[1]
                    if not "-" in form:  # éviter les contractions type "du"
...
def main():
    corpus_train = read_conll("./corpus/train_clean.conll")
    vocab_train = build_vocabulaire(corpus_train)
    for model_name in ("output/model-best", "xx_sent_ud_sm"):
        print(model_name)
        model_spacy = spacy.load(model_name)
        corpus_gold = read_conll("./corpus/test_clean.conll", vocabulaire=vocab_train)
