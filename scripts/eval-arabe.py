...
def main():
    corpus_train = read_conll("./corpus_arabe/corpus-nyuad/ar_nyuad-ud-train.conllu")
    vocab_train = build_vocabulaire(corpus_train)
    for model_name in ("./model-pl/spacy_model2/model-best", "xx_ent_wiki_sm", "xx_sent_ud_sm"):
        print(model_name)
        model_spacy = spacy.load(model_name)
        corpus_gold = read_conll("./corpus_arabe/corpus-nyuad/ar_nyuad-ud-test.conllu", vocabulaire=vocab_train)
